...
Verificação por evidência executada em 2026-07-07 (produção, 9/11 itens); **G1 e G2 fechados por evidência real em 2026-07-08**, após épico 002 (T15/T16/T19) entregar os fixes e o deploy correspondente ir ao ar em produção. **11 de 11 itens fechados → `concluído`.**
...
| 2 | Lead ponta a ponta com variáveis + UTMs | ✅ | 2 leads de teste gravados em produção com `utm_source=li`, `utm_campaign=close-test`; ressalva de cache (ver histórico de G2) fechada em 2026-07-08 — ver evidência de G2 abaixo |
...
| 5 | Preview OG correto LinkedIn/WhatsApp | ✅ | fechado 2026-07-08 via G1 — ver evidência abaixo |
...
| 8 | Merge em `main` → produção sem passo manual | ✅ | deploys automáticos observados; gap de invalidação de cache CloudFront (até 24h de delay) achado e fechado em 2026-07-08 (épico 002, T19) — sem isso este item não era 100% verdade |
| 9 | CI verde (build+vet+test+lint) | ✅ | CI verde após correção de lint; histórico no Actions |
| 10 | Docs refletem o estado entregue | ✅ | `infra/README.md` (runbook), `decisions.md` e este fechamento atualizados em 2026-07-08 |
...
### Tarefas-lacuna — fechadas por evidência em 2026-07-08

- **G1 — OG image PNG (DoD 5). FECHADO.** `og.png`/`og-en.png` (1200×630, PNG) gerados por marketing e servidos por idioma via `ogImageFile(lang)` (épico 002, T15). Evidência: `document.querySelector('meta[property="og:image"]')` em produção retorna `og.png` (PT) e `og-en.png` (EN), com `og:image:width=1200`/`height=630`/`type=image/png`; `HEAD /static/og*.png` confirma `Content-Type: image/png`. **LinkedIn Post Inspector** (`linkedin.com/post-inspector/inspect/https://roosterlabs.com.br/`) renderiza o card com a imagem completa — screenshot capturado em 2026-07-08. Nota upstream para marketing: o inspector emitiu warning não-bloqueante de "description should be at least 100 characters" — copy é decisão de marketing, não travou o fechamento.
- **G2 — Captura de UTM à prova de cache (DoD 2/3). FECHADO.** Causa original: CloudFront ignora query string no cache key e serve HTML com UTMs do primeiro visitante. Fix client-side em `app.js` lendo `location.search` e sobrescrevendo incondicionalmente os 5 hidden inputs + beacon (épico 002, T16). Evidência real em produção (Chrome, 2026-07-08): página carregada com `?utm_source=teste-epico&utm_medium=teste&utm_campaign=fechamento001` populou os campos correspondentes; a MESMA URL sem UTM nenhum, carregada logo em seguida (cache key idêntico, resposta potencialmente cacheada da requisição anterior), veio com os 5 campos vazios — sem vazamento do visitante anterior. Rejeitado: query string no cache key (mata o hit ratio).
- **Achado adicional durante o fechamento (não estava no escopo original de G1/G2):** deploy não invalidava a cache do CloudFront — `cache_policy_id` `CachingOptimized` teimava por até 24h sem `Cache-Control` do Go, então merges em `main` ficavam "no ar" no Lambda mas invisíveis em produção por até um dia. Fechado via épico 002, T19 (`create-invalidation` automático no `deploy.yml`).
...
7. **(fechado no épico 002)** Deploy não invalidava a cache do CloudFront — merge em `main` atualizava o Lambda mas o visitante via a versão anterior por até 24h. Fix: `create-invalidation` automático como último passo do `deploy.yml`.
8. **(fechado no épico 002)** Dependabot, sem contexto da decisão de fixar `golangci-lint-action` em v6/golangci-lint v1, abriu PR pulando pra v9 e quebrou o CI. Fix: `ignore` explícito no `dependabot.yml` até a migração pra golangci-lint v2 ser feita de propósito.

### Nota upstream → marketing

- **OG asset — resolvido.** Marketing gerou `og.png`/`og-en.png` (1200×630 PNG); engineering serviu por idioma no épico 002 (T15). Preview validado no LinkedIn Post Inspector em 2026-07-08.
- **Warning não-bloqueante do LinkedIn Post Inspector:** "the description should be at least 100 characters long" para `og:description`/`meta description` da v0.4. Não travou o fechamento (é só um warning, o card renderiza normalmente), mas é copy — decisão de marketing se vale alongar a descrição.
- **Próximo épico:** copy/estrutura/conversão continuam propriedade de marketing. O épico 002 (sync v0.4 + débito técnico) já está `em-execução`; falta só o T18 (docs) para fechar. Próximo ciclo de melhoria da landing deve ser definido em marketing via `definir-epico`, informado pelos aprendizados acima.

### Handoff

Landing em produção capturando leads qualificados com funil instrumentado; deploy contínuo funcionando; custo ~US$0,50/mês. Sistema provisionado do zero em um dia com 6 defeitos reais encontrados e corrigidos por evidência (entrypoint distroless, política de Host do CloudFront, permissão dupla de Function URL, template parcial, CDN de terceiro no caminho crítico, cache × UTM). G1 (OG PNG) e G2 (UTM à prova de cache) fechados em 2026-07-08 via épico 002 (T15/T16), com um terceiro achado fechado no processo — invalidação de cache no deploy (T19), sem o qual "deploy automático" não era visível em produção por até 24h. Copy agora reflete v0.4 (épico 002). Próximo ciclo: terminar épico 002 (T18, docs) e definir com marketing o próximo ciclo de melhoria da landing.
